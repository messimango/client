    ...
    <div className='flex flex-col justify-center align-middle bg-slate-50'>
      ...
      <div id="products" className='products bg-slate-50 p-4 grid justify-center w-11/12'>
        {selectedCategory === 'all'? <>
          ...
          <article key={item.id} className={`menu-item bg-slate-50 rounded-2xl p-2 m-2`} >
            <img className="menu-item-image border-4 rounded-2xl w-1/3" src={item.imageURL} alt={item.name}/>
            <div className="menu-item-info p-2">
                ...
                <button class="basketButton"><i class="fa fa-shopping-basket"></i>Add To Cart</button>                  
            </div>
            </article>
          ))}
        </> 
        ...
        <article key={item.id} className={`menu-item bg-slate-50 rounded-2xl p-1 m-1 flex flex-row relative`} >
